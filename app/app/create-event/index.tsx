...
import { Check, DollarSign, Upload, Users, X } from "lucide-react-native";
  ...
  return (
    <View className="flex-1 bg-neo-bg">
      ...
      <View className="bg-neo-yellow border-b-4 border-black px-4 pt-12 pb-4 flex-row items-center justify-between z-10">
        <TouchableOpacity
          onPress={handleExit}
          className="bg-neo-red border-2 border-black p-2 active:translate-y-1 shadow-[2px_2px_0px_0px_#000]"
        >
      ...
      <ScrollView contentContainerStyle={{ padding: 20, paddingBottom: 100 }}>
        ...
        >
          <View>
            ...
            <Text className="text-gray-300 font-bold text-xs">
              Fast-track your event creation
            </Text>
          </View>
        ...
        <TouchableOpacity
          onPress={pickCover}
          className="mb-8 w-full h-48 bg-white border-4 border-black border-dashed items-center justify-center shadow-[4px_4px_0px_0px_#000] active:bg-neo-bg"
        >
          {cover ? (
            <Image
              source={{ uri: cover }}
        ...
        <View className="bg-white border-4 border-black p-4 mb-8 shadow-[8px_8px_0px_0px_#000]">
          ...
          {/* COMMUNITY */}
          <Text className="font-bold uppercase text-xs mb-2">
            Host Community
          </Text>
          ...
          <View className="flex-row gap-4 mb-4">
            <View className="flex-1">
              ...
              <TouchableOpacity
                ...
                className="bg-neo-bg border-4 border-black p-3 h-14 justify-center"
              >
                <Text className="font-bold text-black">
                  {start ? start.toLocaleDateString() : "Date"}
                </Text>
              </TouchableOpacity>
              <TouchableOpacity
                onPress={() => showDateTime("start", "time")}
                className="mt-2 bg-neo-bg border-4 border-black p-3 h-14 justify-center"
              >
                <Text className="font-bold text-black">
                  {start
            ...
            <View className="flex-1">
              ...
              >
                <Text className="font-bold text-black">
                  {end ? end.toLocaleDateString() : "Date"}
                </Text>
              </TouchableOpacity>
              <TouchableOpacity
                onPress={() => showDateTime("end", "time")}
                className="mt-2 bg-neo-bg border-4 border-black p-3 h-14 justify-center"
              >
                <Text className="font-bold text-black">
                  {end
        ...
        <View className="bg-white border-4 border-black p-4 mb-8 shadow-[8px_8px_0px_0px_#000] rotate-1">
          ...
          <View className="flex-row gap-2 mb-4 flex-wrap">
            <TouchableOpacity
              ...
              className="bg-black border-2 border-black px-3 py-2 rounded-full active:bg-gray-800"
            >
            ...
            <TouchableOpacity
              onPress={() => setLocationChoice("choose")}
              className="bg-white border-2 border-black px-3 py-2 rounded-full active:bg-neo-yellow"
            >
          ...
          {chosenLocation ? (
            <View className="bg-neo-yellow border-2 border-black p-3 mb-4">
              <Text className="font-black uppercase text-xs mb-1">
                Selections:
              </Text>
        ...
        <View className="bg-white border-4 border-black p-4 mb-8 shadow-[8px_8px_0px_0px_#000]">
          <View className="flex-row justify-between items-center mb-4 border-b-4 border-black pb-2 bg-neo-yellow -mx-4 -mt-4 px-4 pt-4">
            ...
            <TouchableOpacity onPress={() => setPreview(!preview)}>
              <Text className="font-black underline uppercase text-sm">
          ...
          {!preview ? (
            ...
            <View className="bg-neo-bg border-4 border-black p-4 min-h-[200px]">
              <Markdown
                style={{ body: { fontFamily: "SpaceGrotesk_700Bold" } }}
              >
                {description}
              </Markdown>
            </View>
        ...
        <View className="bg-white border-4 border-black p-4 mb-8 shadow-[8px_8px_0px_0px_#000] -rotate-1">
          ...
          <View className="flex-row justify-between items-center mb-4 pb-4 border-b-2 border-gray-200">
            ...
            <Switch
              ...
              trackColor={{ false: "#000", true: "#FFD93D" }}
              thumbColor={requireApproval ? "#000" : "#fff"}
            />
          </View>
          ...
          <View className="flex-row justify-between items-center mb-4 pb-4 border-b-2 border-gray-200">
            ...
            <Switch
              ...
              trackColor={{ false: "#000", true: "#FF6B6B" }}
              thumbColor={isPaid ? "#000" : "#fff"}
            />
          </View>
          {isPaid && (
            <View className="flex-row items-center border-4 border-black p-2 mb-4 bg-neo-bg">
              ...
              <TextInput
                ...
                className="flex-1 font-black text-lg ml-2"
              />
            </View>
          )}
          ...
          <View className="flex-row mb-4">
            <TouchableOpacity
              ...
              className={`flex-1 border-2 border-black p-3 items-center ${isPublic ? "bg-black" : "bg-white"}`}
            >
              <Text
                className={`font-black uppercase ${isPublic ? "text-white" : "text-black"}`}
              >
                Public
              </Text>
            </TouchableOpacity>
          ...
          <View className="flex-row mb-4">
            <TouchableOpacity
              ...
              className={`flex-1 border-2 border-black p-3 items-center ${unlimited ? "bg-black" : "bg-white"}`}
            >
              <Text
                className={`font-black uppercase ${unlimited ? "text-white" : "text-black"}`}
              >
                Unlimited
              </Text>
            </TouchableOpacity>
